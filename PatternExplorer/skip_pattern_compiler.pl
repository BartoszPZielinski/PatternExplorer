...
/*
    ...
    auto{
         transitions: [trans(V, Type, pos(List), S0, S1):-G, ...],
         epses: [eps(S0, S1), ...],
...
comp_aut(iter(P), auto{
   ...
   --> current_vars(Vs),
       ...
       replace_vars(_,Vs),
       {
         ...
         %sources_target_acc_epses(Fin, IterFinal, Auto0.epses, Es0),
         %sources_target_cond_acc_epses(Fin, I1, 
         %   CounterVar #= CounterVar0 + 1, Es0, Es)
         sources_target_cond_acc_epses(Fin, I1, 
            CounterVar #= CounterVar0 + 1, Auto0.epses, Es0),
         sources_target_acc_epses(Fin, IterFinal, Es0, Es)
       }.
